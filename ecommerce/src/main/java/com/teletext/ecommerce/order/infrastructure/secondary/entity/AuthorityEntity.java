...
import annotations.com.teletext.ecommerce.order.infrastructure.secondary.entity.AuthorityEntityBuilder;
import com.teletext.ecommerce.order.domain.user.aggregate.Authority;
...
public class AuthorityEntity implements Serializable {
    ...
    public static Set<Authority> toDomain(Set<AuthorityEntity> authorityEntities) {
       return authorityEntities.stream()
              .map(authorityEntity -> AuthorityBuilder.authority().name(new AuthorityName(authorityEntity.name))
                      .build())
